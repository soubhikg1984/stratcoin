const StratfitSmartToken = artifacts.require('../contracts/StratfitSmartToken.sol');
const utils = require('./helpers/Utils');

contract('StratfitSmartToken', (accounts) => {
    let token;
    let owner = accounts[0];

    beforeEach(async () => {
        token = await StratfitSmartToken.new();
    });
    ...
    describe('construction', async () => {
        ...
        it('should return correct name after construction', async () => {
            assert.equal(await token.name(), "STRATCOIN");
        });

        it('should return correct symbol after construction', async () => {
            assert.equal(await token.symbol(), 'SRT');
        });
        ...
        it('should throw when attempting to transfer by default', async () => {
            let token = await StratfitSmartToken.new();
