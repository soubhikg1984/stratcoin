...
//const StratfitCrowdsale = artifacts.require('StratfitCrowdsale.sol')
const StratfitCrowdsale = artifacts.require('../contracts/StratfitCrowdsale.sol')
...
contract('StratfitCrowdsale', function([_, investor, owner, wallet, walletTeam, walletOEM, walletBounties, walletReserve]) {
    ...
    beforeEach(async function() {
        ...
        this.token = await StratfitSmartToken.new({from: owner});
        this.refundVault = await RefundVault.new(wallet, this.token.address,{from: owner});

        this.crowdsale = await StratfitCrowdsale.new(this.startTime,
            this.endTime,
            wallet,
            walletTeam,
            walletOEM,
    ...
    describe('Rate Mechanism', function() {
        ...
        it('Should be on day 1 - 7000 ', async function() {
            let rate = await this.crowdsale.getRate.call()
            assert.equal(rate, 7000);
        });

        it('Should be on day 2 - 6500 ', async function() {
            await increaseTimeTo(this.startTime + duration.days(7));
            let rate = await this.crowdsale.getRate.call()
            assert.equal(rate, 6500);
        });

        it('Should be on day 3 - 6000 ', async function() {
            await increaseTimeTo(this.startTime + duration.days(14));
            let rate = await this.crowdsale.getRate.call()
            assert.equal(rate, 6000);
        });

        it('Should be on day 4 - 5500 ', async function() {
            await increaseTimeTo(this.startTime + duration.days(21));
            let rate = await this.crowdsale.getRate.call()
            assert.equal(rate, 5500);
        });

        it('Should be on day 5 - 5000 ', async function() {
