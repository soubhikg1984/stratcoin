...
import './StratfitSmartToken.sol';


contract StratfitCrowdsale is FinalizableCrowdsale {

    // =================================================================================================================
    //                                      Constants
    // =================================================================================================================
    // Max amount of known addresses of which will get SRT by 'Grant' method.
    //
    // grantees addresses will be Stratfit wallets addresses.
    // these wallets will contain SRT tokens that will be used for 2 purposes only -
    // 1. SRT tokens against raised fiat money
    // 2. SRT tokens for presale bonus.
    ...
    // SRT to ETH base rate
    uint256 public constant EXCHANGE_RATE = 5000;
    ...
    // wallets address for 60% of SRT allocation
    address public walletTeam;   //10% of the total number of SRT tokens will be allocated to the team
    address public walletOEM;       //10% of the total number of SRT tokens will be allocated to OEM’s, Operating System implementation, SDK developers and rebate to device and Shield OS™ users
    address public walletBounties;  //5% of the total number of SRT tokens will be allocated to professional fees and Bounties
    address public walletReserve;   //35% of the total number of SRT tokens will be allocated to STRATFIT LABS and as a reserve for the company to be used for future strategic plans for the created ecosystem
    ...
    function StratfitCrowdsale(uint256 _startTime,
    uint256 _endTime,
    address _wallet,
    address _walletTeam,
    address _walletOEM,
    ...
    StratfitSmartToken _StratfitSmartToken,
    RefundVault _refundVault)
    public
    Crowdsale(_startTime, _endTime, EXCHANGE_RATE, _wallet, _StratfitSmartToken) {
        ...
        require(_StratfitSmartToken != address(0));
    ...
    // @return the rate in SRT per 1 ETH according to the time of the tx and the SRT pricing program.
    // @Override
    function getRate() public view returns (uint256) {
        if (now < (startTime.add(7 days))) {return 7000;}
        if (now < (startTime.add(14 days))) {return 6500;}
        if (now < (startTime.add(21 days))) {return 6000;}
        if (now < (startTime.add(28 days))) {return 5500;}
        if (now < (startTime.add(42 days))) {return 5000;}
    ...
    function finalization() internal onlyOwner {
        ...
        // 10% of the total number of SRT tokens will be allocated to the team
        token.issue(walletTeam, newTotalSupply.mul(10).div(100));

        // 10% of the total number of SRT tokens will be allocated to OEM’s, Operating System implementation,
        // SDK developers and rebate to device and STRATFIT OS™ users
        token.issue(walletOEM, newTotalSupply.mul(10).div(100));

        // 5% of the total number of SRT tokens will be allocated to professional fees and Bounties
        token.issue(walletBounties, newTotalSupply.mul(5).div(100));

        // 35% of the total number of SRT tokens will be allocated to STRATFIT LABS,
    ...
    /// @dev Accepts new ownership on behalf of the StratfitCrowdsale contract. This can be used, by the token sale
    /// contract itself to claim back ownership of the StratfitSmartToken contract.
